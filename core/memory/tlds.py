"""IANA top-level domains, for the well-formed check on ``domain`` / ``email``.

The Python half of ``services/agent/workflows/hunt/tlds.ts``: the same list, the
same exclusions, so text extracted here and text extracted by the hunt harness
agree on what is a domain. Re-vendor both files together.
"""
...
# Vendored from https://data.iana.org/TLD/tlds-alpha-by-domain.txt
# Version 2026080600, Last Updated Thu Aug  6 07:07:01 2026 UTC
# fmt: off
_IANA: Tuple[str, ...] = (
...
# Real TLDs that are also common file extensions. In security telemetry a
# backup.zip is a file far more often than a domain, so prose drops these.
_FILE_EXTENSIONS: FrozenSet[str] = frozenset(
    ["zip", "mov", "js", "py", "sh", "pl", "cab", "prof"]
)

TLDS: FrozenSet[str] = frozenset(_IANA) - _FILE_EXTENSIONS
