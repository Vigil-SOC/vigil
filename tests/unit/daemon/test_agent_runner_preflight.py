...
def _run(coro):
    ...
    # raises "no current event loop" once any earlier test has called
    # asyncio.run, which closes the loop and clears it from the thread. That
    # made these tests depend on collection order.
