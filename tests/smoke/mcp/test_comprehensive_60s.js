...
(async () => {
  ...
  try {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      if (/deeptempo|tempo-flow|approval/i.test(chatContent)) indicators.push('SERVERS');
    ...
    const hasServerNames = /deeptempo-findings|tempo-flow|approval/i.test(aiResponse);
    ...
    const serverMatches = aiResponse.match(/deeptempo-findings|tempo-flow|approval|github|crowdstrike|sentinelone/gi) || [];
