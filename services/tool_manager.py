...
# Both agent loops route here so the tool-name -> handler mapping can't drift
# between them (the OpenAI loop previously had its own copy that called
# DatabaseDataService methods by tool name and silently fell through for the
# 6 tools whose names don't match a method).
